...
Load_SP <- reactive({
  ...
  if(input$SP_CK==TRUE){
    if(input$SP_CT==TRUE){
      SP_Data <- Pick_SP(projectSP,input$SP_PC,input$SP_RC)
      ...
      SP_Data <- Pick_SP(projectSP,1,NULL)
      SP_Data <- RegMatch_SP(SP_Data,RegSource$TG_RS,RegSource$TM_RS,RegSource$MG_RS,RegSource$MT_RS)
      if(nrow(SP_Data)!=0){
        SP_Data <- SP_Data[,2:18]
        #return full interactions of sub-interactions
...
Load_TFFL <- reactive({
  ...
  if(input$TFFL_CK==TRUE){
    if(input$TFFL_CT==TRUE){
      TFFL_Data <- Pick_TFFL(projectTFFL,input$TFFL_PC,input$TFFL_RC)
      ...
      TFFL_Data <- Pick_TFFL(projectTFFL,1,NULL)
      TFFL_Data <- RegMatch_TFFL(TFFL_Data,RegSource$TG_RS,RegSource$TM_RS,RegSource$MG_RS,RegSource$MT_RS)
      if(nrow(TFFL_Data)!=0){
        TFFL_Data <- TFFL_Data[,2:18]
        #return full interactions of sub-interactions
...
Load_MFFL <- reactive({
  ...
  if(input$MFFL_CK==TRUE){
    if(input$MFFL_CT==TRUE){
      MFFL_Data <- Pick_MFFL(projectMFFL,input$MFFL_PC,input$MFFL_RC)
      ...
      MFFL_Data <- Pick_MFFL(projectMFFL,1,NULL)
      MFFL_Data <- RegMatch_MFFL(MFFL_Data,RegSource$TG_RS,RegSource$TM_RS,RegSource$MG_RS,RegSource$MT_RS)
      if(nrow(MFFL_Data)!=0){
        MFFL_Data <- MFFL_Data[,2:18]
        #return full interactions of sub-interactions
...
Load_CFFL <- reactive({
  ...
  if(input$CFFL_CK==TRUE){
    if(input$CFFL_CT==TRUE){
      CFFL_Data <- Pick_CFFL(projectCFFL,input$CFFL_PC,input$CFFL_RC)
      ...
      CFFL_Data <- Pick_CFFL(projectCFFL,1,NULL)
      CFFL_Data <- RegMatch_CFFL(CFFL_Data,RegSource$TG_RS,RegSource$TM_RS,RegSource$MG_RS,RegSource$MT_RS)
      if(nrow(CFFL_Data)!=0){
        CFFL_Data <- CFFL_Data[,2:18]
        #return full interactions of sub-interactions
...
#' =============================================================
#' @description ForceNet_UI: the UI of network default
#' =============================================================
# output$NetDefault_UI <- renderUI({
#   fluidPage(
#     imageOutput(outputId = "InIsNullPNG",
#                 width = "100%",
#                 height = "auto")
#   )
# })  
...
#' =============================================================
#' @description default output when input is null
#' =============================================================
# output$InIsNullPNG <- renderImage({
#   filename <- normalizePath(file.path("./www/background2.png"))
...
output$GEDT <- DT::renderDataTable(server = F,{
 ...
 # print(GE_DT$Symbol)
  ...
  GE_DT <- GE_DT[which(GE_DT$pvalue<=input$GE_Pcutoff),]
  DT::datatable(GE_DT, 
                extensions = 'Buttons', 
                options = list(
                  pageLength=10,
...
output$miREDT <- DT::renderDataTable({
  miR_list <- miR_Parse()
  # print(TG_list)
  # print(length(TG_list))
  validate(
  ...
  miRE_DT <- miRE_DT[which(miRE_DT$pvalue<=input$miRE_Pcutoff),]
